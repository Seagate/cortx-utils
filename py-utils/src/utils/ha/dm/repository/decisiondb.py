...
class DecisionDB:
    ...
    def __init__(self) -> None:
        ...
        if os.path.exists(os.path.join(const.CONF_PATH, const.HA_DATABASE_SCHEMA)):
            schema = Json(os.path.join(const.CONF_PATH, const.HA_DATABASE_SCHEMA)).load()
            conf = GeneralConfig(schema)
        else:
            conf = GeneralConfig(database.DATABASE)
        self.storage = DataBaseProvider(conf)

    async def store_event(self, entity, entity_id, component, component_id, alert_time, action):
        """
        ...
        # Generate Key
        decision_id = DecisionModel.create_decision_id(entity, entity_id, component, component_id,
                                                       alert_time)
        Log.debug(f"Loading event for {decision_id} Action:- {action}")
        # Generate Decision DB Object.
        decision = DecisionModel.instantiate_decision(decision_id=decision_id, action=action,
    ...
    async def get_event(self, entity, entity_id, component, component_id,
        ...
        # Create Query
        query = Query().filter_by(Compare(DecisionModel.decision_id, '=', decision_id))
        return await self.storage(DecisionModel).get(query)

    async def get_event_time(self, entity, entity_id, component, component_id, **kwargs):
        """
        ...
        # Create Query
        query = Query().filter_by(Compare(DecisionModel.decision_id, 'like', decision_id))
