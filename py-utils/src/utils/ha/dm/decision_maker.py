...
class RuleEngine:
    ...
    def _load_rules(self):
        """Reads the json structured rule data from the file, and returns it in dict format."""
        rules_data = None
        try:
    ...
    def evaluate_alert(self, alert):
        """
        Compares the alert with the predefined json structure and returns action such as fail-over,
        fail-back etc.
        """
        ...
        if res_type == const.IEM:
            ...
            module_var = sensor_response.get(const.SPECIFIC_INFO).get(const.SPECIFIC_INFO_MODULE)
        res_type_data = self._rules_schema[res_type]
        if res_type_data is not None:
            for item in res_type_data:
                if alert_type == item[const.ALERT_TYPE] and severity == item[const.SEVERITY]:
                    if res_type == const.IEM:
                        if (component_var == item[const.COMPONENT_ID]
                                and module_var == item[const.MODULE_ID]):
                            action = item[const.ACTION]
                    else:
                        action = item[const.ACTION]
        Log.debug(f"Found {action} action for resource: {res_type} with alert type: "
                  f"{alert_type} and severity: {severity}.")
        return action
...
class DecisionMaker:
    ...
    """
    This class is responsible for taking the HA decisions such as failover/failback with the help of
    RuleEngine
    """
    ...
    def __init__(self, decisiondb=DecisionDB()):
        ...
        self._conf = Json(os.path.join(const.CORTX_HA_INSTALL_PATH, const.CONF_FILE_PATH)).load()

    async def _get_data_nw_interface(self, host_id):
        interface = []
        if self._conf:
            interface = self._conf.get(const.NETWORK).get(host_id).get(const.DATA_IFACE)
        return interface

    async def _get_mgmt_nw_interface(self, host_id):
        interface = []
        if self._conf:
            interface = self._conf.get(const.NETWORK).get(host_id).get(const.MGMT_IFACE)
        return interface
    ...
    async def handle_alert(self, alert):
        ...
        try:
            ...
            Log.error(f"Error occurred during alert handling. {e}")
    ...
    async def _store_action(self, alert, action):
        ...
        try:
            ...
            if info_dict:
                await self._decision_db.store_event(
                    info_dict[const.ENTITY], info_dict[const.ENTITY_ID], info_dict[const.COMPONENT],
                    info_dict[const.COMPONENT_ID], info_dict[const.EVENT_TIME], action)
        except Exception as e:
            Log.error(f"Error occurred during storing action. {e}")
    ...
    async def _set_db_key_info(self, sensor_response):
        ...
        Key format - HA/entity/entity_id/component/component_Id/timestamp
        Examples -
        ...
        if info_dict[const.COMPONENT] == const.CONTROLLER:
            ...
            # For IEM the component id is fetched from specific info's component id field
    ...
    async def _get_component_id_for_nic(self, host_id, resource_id):
        ...
        if resource_id in nw_interface:
            ...
            # Since resource not found in data_nw lets search is mgmt_nw.
