...
class ElasticsearchV:
    ...
    def validate_service_status(self, host, port):
        """Validate Elasticsearch service status."""
        ...
        try:
            ...
            if status_code != 200:
                raise VError(
                    ...
                    f"connecting to elasticsearch service on {host}:{port}."))
            ...
            raise VError(errno.ECONNREFUSED,
                         f"No Elasticsearch service running on {host}:{port}")
        except requests.exceptions.RequestException as req_ex:
            raise VError(
                errno.ECOMM, (f"Error connecting to Elasticsearch service on "
                f"{host}:{port}. {req_ex}"))
