...
class Log:
    ...
    def init(service_name, log_path, level="INFO", backup_count=10, file_size_in_mb=10,
        ...
        Log.audit_logger = Log._get_logger(
            syslog_server, syslog_port, service_name, "audit", getattr(Log, level), log_path,
            backup_count, max_bytes)

        Log.logger = Log._get_logger(
            syslog_server, syslog_port, service_name, "system", getattr(Log, level), log_path,
            backup_count, max_bytes)
    ...
    def _get_logger(syslog_server: str, syslog_port: str, file_name: str, logger_type,
        ...
        :param file_name: File name for Log Files.
        ...
        if syslog_server and syslog_port:
            log_handler = logging.handlers.SysLogHandler(address=(syslog_server, syslog_port))
    ...
    def trace_method(level, exclude_args=[], truncate_at=80):
        ...
        def _print_start(func, *args, **kwargs):
            ...
            kw_args = \
                [f"{key}={_fmt_value(kwargs[key])}" for key in kwargs if key not in exclude_args]
        ...
        def _print_end(func, response):
            message = f"[{func.__qualname__}] Returned {_fmt_value(response)}"
            Log.logger.log(level, message)
